...
The marginal cost is increasing with the quantity of electricity generation through the utilization rate $\Omega_{k} = \frac{Q_{k}}{Cap_{k}}$. Static decreasing returns are assumed in every sector of IMACLIM-R. They are associated with lower labor productivity (see doc). %starting form here, we  write two versions: what is in the code and what is should look like
%current
