...
IMC_reg <- c("USA","CAN","EUR","JAN","CEI","CHN","IND","BRA","MDE","AFR","RAS","RAL")
...
for(i in var_list){
  ...
  var_total_desc[[i]] <- c(read_csv(paste0("../figures&tables/supplementary/sources_",i,".csv"), col_names = FALSE,col_types = "c"))
...
export_kable_latex <- function(x,y,cap){
  kbl(x, format="latex", caption = cap, booktabs = T, digits = 2, longtable = T) %>%
    kable_styling(latex_options = c("striped","hold_position"), font_size = 8)%>% 
    footnote( general = y,
          general_title = "Sources:",title_format = c("italic", "underline"), footnote_as_chunk = T,threeparttable=T)
}
